	...
	def __pass_data_to_option(self, data: list[Playable]) -> None:
		...
		for track in data:
			self._selector.append_option(
				SelectOption(
					label=YggUtil.truncate_string(track.title, max=100),
					description=f"{YggUtil.truncate_string(track.author, max=50)}\
                      - {UtilTrackPlayer.parse_sec(track.length)}",
	...
	def get_embed(self) -> Embed:
		...
		for track in data:
			...
			embed.description += f"{count}. **[{
				YggUtil.truncate_string(f'{track.title} | {track.author}')
