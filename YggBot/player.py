...
class TrackView(View):
    ...
    def __is_previous_disabled(self) -> bool:
        ...
        if (
            self._player.queue.history[self._player.queue.history.count - 1]
            is self._player.current
            and self._player.queue.history.count <= 1
        ):
            return True
    ...
    async def interaction_check(self, interaction: Interaction) -> bool:
        ...
        if isTrue:
            self._track_control._record_timestamp(
                guild_id=interaction.guild_id, interaction=interaction
            )
...
class SelectView(View):
    ...
    def __pass_data_to_option(self) -> None:
        ...
        for index, track in enumerate(self._data):
            ...
            self._selector.options.append(
                SelectOption(
                    label=track.title,
                    description=f"{track.author if not isinstance(track, SpotifyTrack) else track.artists}\
                      - {MusicPlayerBase._parseSec(track.duration)}",
    ...
    async def _selector(self, interaction: Interaction, select: Select) -> None:
        ...
        embed: Embed = await self._track_control._play_response(
            ...
            is_put_front=True if self._is_jump_command else False,
            is_autoplay=self._autoplay,
        )

        if not self._is_jump_command:
            _, _, _ = await self._track_control.play(
                interaction, query=self._selected, autoplay=self._autoplay
            )
        else:
...
class MusicPlayerBase:
            ...
            if YggUtil.get_time() >= (
                ...
                if not guild.id == YggConfig.KANTIN_YOYOK_ID:
                    ...
                    if not voice_client.is_playing() and not voice_client.is_paused():
                        interaction: Interaction = self._guilds[id][
                            "interaction"
                        ]
                        ...
                        self._guilds[id]["timestamp"] = YggUtil.get_time()
    ...
    async def _play_response(
        ...
        member: Member,
        /,
        track: Playlist | Playable | SpotifyTrack | list[SpotifyTrack],
        is_playlist: bool = False,
        ...
        embed.set_footer(text=f"From {member.name} ",
        ...
        async def _get_raw_spotify_playlist(uri_ori: str) -> dict:
            ...
            async with self._bot.session.get(
                uri, headers=sp_client.bearer_headers
            ) as resp:
                if resp.status == 400:
                    return None
    ...
    def _record_timestamp(self, guild_id: int, interaction: Interaction) -> None:
        ...
        if not "timestamp" in self._guilds[guild_id]:
            self._guilds[guild_id].update({"timestamp": YggUtil.get_time()})

        self._guilds[guild_id].update({"interaction": interaction})
    ...
    async def on_wavelink_websocket_closed(
        ...
        if payload.player.is_playing() or payload.player.is_paused():
            await wait([create_task(self.__clear_message(payload.player.guild.id))])
...
class MusicPlayer(MusicPlayerBase):
    ...
    async def search(
        ...
        tracks: Playable | Playlist | SpotifyTrack | list[SpotifyTrack] = await self._custom_wavelink_player(
    ...
    async def play(
        ...
        /,
        query: str | Playable | SpotifyTrack,
        source: TrackType = TrackType.YOUTUBE,
        autoplay: bool = None,
        force_play: bool = False,
        put_front: bool = False,
    ) -> Tuple[Playable | Playlist | SpotifyTrack, bool, bool]:
        ...
        if isinstance(tracks, Playlist | list):
            ...
            if put_front or force_play:
                if player.is_playing():
                    ...
                    player.queue._queue.extendleft(
    ...
    async def previous(self, interaction: Interaction) -> bool:
        ...
        if (
            ...
            and player.queue.history[player.queue.history.count - 1] is player.current
        ):
            player.queue.put_at_front(player.queue.history.pop())
            player.queue.put_at_front(player.queue.history.pop())
            await player.seek(player.current.length * 1000)
